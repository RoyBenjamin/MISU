...
# This file is needed as I fixed an issue with AUCM_MultiLabel
...
class AUCMLoss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        y_pred = y_pred.reshape(-1, 1)  # be careful about these shapes
...
class AUCM_MultiLabel(torch.nn.Module):
    ...
    def __init__(self, margin=1.0, imratio=None, num_classes=1, device=None):
        ...
        self.alpha = torch.zeros(num_classes, dtype=torch.float32, device=self.device, requires_grad=True)\
            .to(self.device)

    @property
    def get_a(self):
        return self.a.mean_layer()

    @property
    def get_b(self):
        return self.b.mean_layer()

    @property
    def get_alpha(self):
        return self.alpha.mean_layer()

    def forward(self, y_pred, y_true, mask=None):
        total_loss = 0
        for idx in range(self.num_classes):
            if mask is not None:
                mask_i = mask[:, idx].reshape(-1, 1)
...
class APLoss_SH(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true, index_s):
        ...
        if f_ps.size(0) == 1:
            self.u_pos[index_s] = (1 - self.beta) * self.u_pos[index_s] + self.beta * (pos_loss.mean_layer())
            self.u_all[index_s] = (1 - self.beta) * self.u_all[index_s] + self.beta * (loss.mean_layer())
        else:
            self.u_all[index_s] = (1 - self.beta) * self.u_all[index_s] + self.beta * (loss.mean_layer(1, keepdim=True))
            self.u_pos[index_s] = (1 - self.beta) * self.u_pos[index_s] + self.beta * (pos_loss.mean_layer(1, keepdim=True))
